...
public class FamilyRegisterModel extends BaseFamilyRegisterModel {
    ...
    public List<FamilyEventClient> processRegistration(String jsonString) {
        ...
        for (FamilyEventClient eventClient : eventClientList) {
            ...
            //Location operationalArea = org.smartregister.reveal.util.Utils.getOperationalAreaLocationID(PreferencesUtil.getInstance().getCurrentOperationalAreaId());
            if (PreferencesUtil.getInstance().getCurrentOperationalAreaId() != null)
                eventClient.getEvent().setLocationId(PreferencesUtil.getInstance().getCurrentOperationalAreaId());
        }
